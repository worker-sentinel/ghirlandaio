...
pacstrap /mnt pacstrap /mnt base intel-ucode linux-lt linux-lts-headers linux-firmware mkinitcpio lvm2 sudo curl neovim iwd firewalld pacman docker networkmanager 

!// gunakan NetworkManager untuk admin dan server, yang nantnya untuk configurasi IP dan izin akses admin dan server atau worker ke dalam podman dan configurasi firewalld 
...
!// buat dua user dalam pengguaan berbeda, user untuk server dan operator <br>

server 

useradd -m [namauser-untuk-servermu] <br>
passwd [namausermu] <br>
echo "[namausermu] ALL=(ALL:ALL) ALL" >> /etc/sudoers.d/terserahkamunamanyaapa <br>

operator 

useradd -m [namauser-untuk-operator-atau-workermu] <br>
passwd [namaoperator-atau-worker] <br> 

! // operator tidak di berikan hak sudo atau mengubah sistem selain untuk menjalankan dan mengeerajakan bagian service dan pemakaian server <br>
...
systemctl enable iwd  // jika menggunakan iwd <br>
systemctl enable NetworkManager //jika menggunakan networkmanager <br>

!// disable salah satu DHCP dari iwd atau NetworkManager agar tidak berebutan wifi card <br>
!// untuk config firewalld dan podman disarankan atau dapat menggunakan networkmanager  <br>

## disable service 

systemctl stop iwd <br>
systemctl disable iwd <br>
