...
lvcreate -L 8G (sagi) -n home
lvcreate -L 17.7G (sagi) -n podman
```
## Memformat lvm
```
...
mount --mkdir -o rw,nodev,nosuid,relatime /dev/sagi/vars /mnt/var
...
lsblk
```
## install package (kernel hardened & firmware, networking, firewall, container, asciinema, dll)
```
pacstrap /mnt amd-ucode base linux-hardened linux-hardened-headers linux-firmware mkinitcpio lvm2 sudo pacman git wget curl neovim iwd firewalld openssh grep podman podman-compose asciinema
```
## FSTAB
```
...
## sudo untuk user
```
echo "sagi1 ALL=(ALL:ALL) ALL" > /etc/sudoers.d/sagi1
```
## Kernel
```
mkdir /etc/cmdline.d
touch /etc/cmdline.d/{01-boot.conf,02-misc.conf}
echo "rd.luks.name=$(blkid -s UUID -o value /dev/[partisi root])=(nama device) root=/dev/(nama volume)/root" > /etc/cmdline.d/01-boot.conf
