...
```
ssh crew@192.168.1.3
```

Melakukan remote akses menggunakan SSH ke server dengan IP ``` 192.168.1.3 ``` menggunakan username ``` crew ```

```
password
```

Memasukkan kata sandi untuk autentikasi user crew secara manual.
...
Menampilkan seluruh konfigurasi antarmuka jaringan (IP address, MAC address, status interface) pada server target. Biasanya digunakan untuk memastikan IP ``` 192.168.1.3 ``` sudah terpasang dengan benar.

```
exit
```

Keluar dari sesi Root kembali menjadi user biasa ``` crew ```.

```
exit
```

Menutup koneksi SSH dan keluar dari server ``` 192.168.1.3 ``` kembali ke komputer lokal/master.
...
masuk kembali ke server ``` .3 ``` untuk menginstal K3s dengan peran sebagai Agent (Worker Node).
...
Keluar dari Root dan mengakhiri sesi SSH di server ``` .3 ```.
...
Menampilkan daftar semua node (baik master maupun worker) yang terhubung di dalam cluster Kubernetes. Di tahap ini, memastikan apakah node ``` 192.168.1.3 ``` tadi sudah berstatus ``` Ready ``.

## 5. Investigasi Konfigurasi Internal K3s

Kembali masuk ke server ``` .3 ``` karena mendeteksi sesuatu atau ingin memeriksa konfigurasi load balancer bawaan K3s.
...
Membaca isi file konfigurasi JSON milik load balancer internal K3s Agent. File ini berisi informasi bagaimana agent (worker) tersebut mendistribusikan trafik atau menghubungi master node (server ``` .3 ```).
