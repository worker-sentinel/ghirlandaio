...
```
```
mount --mkdir -o uid=0,gid=0,fmask=0077,dmask=0077 /dev/vda1 /mnt/boot
```
...
```
```
mkfs.ext4 /dev/proc/podi
```
```
mount --mkdir -o rw,nodev,nosuid,relatime /dev/proc/podi /mnt/var/lib/containers
```

## home internal
```
lvcreate -l100%FREE proc -n priv
```
```
...
# packages
```
pacstrap /mnt intel-ucode linux-lts linux-lts-headers linux-firmware mkinitcpio lvm2 base sudo curl neovim iwd firewalld pacman which grep podman openssh pam_mount
```
# fstab
```
...
```
useradd -d /home/user haktor
```

```
chown -R haktor:haktor /home/user
```
...
```
echo 'haktor ALL=(ALL:ALL) ALL' > /etc/sudoers.d/none
```
