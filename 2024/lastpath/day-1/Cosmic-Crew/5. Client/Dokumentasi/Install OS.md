...
```
vgcreate keju /dev/mapper/proc
```
Membuat Volume Group bernama slims.

# Setup LVM
```
lvcreate -L 15G keju -n root 
```
...
>/dev/ keju /root: Lokasi partisi root yang ingin diformat

```
mkfs.ext4 /dev/ keju /vars
```
Menyiapkan volume untuk menyimpan data sistem.
>mkfs.ext4: singkatan dari "Make File System ext4" untuk memformat partisi menjadi sistem file ext4

>/dev/ keju /vars: Lokasi partisi vars yang ingin diformat

```
mkfs.ext4 /dev/ keju /vtmp
```
Menyiapkan tempat penyimpanan file sementara.
>mkfs.ext4: singkatan dari "Make File System ext4" untuk memformat partisi menjadi sistem file ext4

>/dev/ keju /vars: Lokasi partisi vtmp yang ingin diformat

```
mkfs.ext4 /dev/ keju /vlog
```
Menyiapkan tempat penyimpanan log sistem.
>mkfs.ext4: singkatan dari "Make File System ext4" untuk memformat partisi menjadi sistem file ext4

>/dev/ keju /vlog: Lokasi partisi vlog yang ingin diformat

```
mkfs.ext4 /dev/ keju /vaud
```
Menyiapkan tempat penyimpanan log audit.
>mkfs.ext4: singkatan dari "Make File System ext4" untuk memformat partisi menjadi sistem file ext4

>/dev/ keju /vaud: Lokasi partisi vaud yang ingin diformat

```
mkfs.ext4 /dev/ keju /home
```
Menyiapkan tempat penyimpanan data pengguna.
>mkfs.ext4: singkatan dari "Make File System ext4" untuk memformat partisi menjadi sistem file ext4

>/dev/ keju /home: Lokasi partisi home yang ingin diformat

```
mkfs.ext4 /dev/ keju /podman
```
Menyiapkan tempat penyimpanan data Podman.
>mkfs.ext4: singkatan dari "Make File System ext4" untuk memformat partisi menjadi sistem file ext4

>/dev /keju /podman: Lokasi partisi podman yang ingin diformat
...
>/dev/ keju /root: Lokasi partisi root yang akan dimounting
...
>/dev/ keju /vars: Lokasi partisi vars yang akan dimounting

>/mnt/var: Folder target tempat partisi var berada

```
mount --mkdir -o rw,nodev,nosuid,noexec,relatime /dev/ keju /vtmp /mnt/var/tmp     
```
...
>/dev/ keju /vtmp: Lokasi partisi vtmp yang akan dimounting

>/mnt/var/tmp: Folder target tempat partisi tmp berada

```
mount --mkdir -o rw,nodev,nosuid,noexec,relatime /dev/ keju /vlog /mnt/var/log  
```
...
>/dev/ keju /vlog: Lokasi partisi vlog yang akan dimounting

>/mnt/var/log: Folder target tempat partisi log berada

```
mount --mkdir -o rw,nodev,nosuid,noexec,relatime /dev/ keju /vaud /mnt/var/log/audit      
```
...
>/dev/ keju /vaud: Lokasi partisi vaud yang akan dimounting

>/mnt/var/log/audit: Folder target tempat partisi audit berada

```
mount --mkdir -o rw,nodev,nosuid,relatime /dev/ keju /podman /mnt/var/lib/container
```
...
```
mount --mkdir -o rw,nodev,nosuid,relatime /dev/ keju /home /mnt/home
```
Untuk menyimpan data pengguna.
>mount: Mengaktifkan sistem file atau partisi ke dalam sebuah direktori agar bisa diakses lewat file manager atau terminal
...
>/dev/ keju /home: Lokasi partisi home yang akan dimounting
...
## Menambahkan User
Menambahkan user 'feby' dengan home directory
```
useradd -d /home/user gass
```
Menambahkan user baru bernama feby.
...
passwd feby
```
Membuat password untuk user feby.
