## Connect Wifi untuk menyambungkan internet
```bash iwctl
station wlan0 [sesuaikan device list] get-networks
station wlan0 connect "nama wifi"
exit
```
memeriksa jaringan sudah tersambung atau belum
...
### Fungsi
1. Boot untuk menyimpan file yang dibutuhkan saat laptop dinyalakan, tanpa boot laptop tidak bisa masuk ke masuk ke sistem operasi
2. Root untuk menyimpan semua file Arch Linux
3. Vars untuk menyimpan data yang sering berubah dan file file yang terus diperbarui ketika sistem masih berjalan
4. vtmp untuk menyimpan file sementara yang dibuat oleh aplikasi
5. vlog untuk menyimpan catatan aktivitas sistem seperti koneksi jaringan, dan sistem error
6. vaud untuk menyimpan catatan keamanan seperti siapa yang login atau siapa yang menggunakan sudo
7. home untuk menyimpan data pengguna seperti dokumen, foto, file
...
## Install package
```
pacstrap /mnt amd-ucode linux-lts linux-lts-headers linux-hardened linux-hardened-headers linux-firmware lvm2 base base-devel neovim openssh superfile podman podman-desktop iptables mpd mpc mpv keepassxc secrets booster networkmanager pam_mount
```
